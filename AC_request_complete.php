<?php
	...
	$head= $_POST['head'];
	$item = $_POST['item'];
	$current_userid = $_POST['curruser'];
		
	include 'secure/connectstring.php';
	
	//Update request_status of tb_requests_item to "DELETED"
	
	$queryupdate = "UPDATE tb_request_info SET req_status='COMPLETED' WHERE req_id='$head' AND req_item_id='$item'";
	mysql_query($queryupdate) or die(mysql_error());
	
	
	//Add entry in tb_requests_status
	$newstatus = "COMPLETED";
